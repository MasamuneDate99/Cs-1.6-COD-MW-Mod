...
new const nazwa[] = "Desolator";
new const opis[] = "You get sentry";
...
public plugin_init()
{
	...
	pcvarHealth = register_cvar("inzynier_health","300")
...
public plugin_precache()
{
	...
	precache_model("models/rgcod/v_tfc_spanner.mdl")
	precache_model("models/rgcod/base2.mdl")
	precache_model("models/rgcod/sentry2.mdl")
...
public OpcjeDzialka(id)
{
	...
	gMenuDzialko[id] = menu_create("Gun", "OpcjeDziala_Handle");
	menu_additem(gMenuDzialko[id],"Build");
	menu_additem(gMenuDzialko[id],"Destroy");
	...
	switch(item)
	{
		case 0:
		{
			if(ma_perk[id] && ma_dzialko[id])
			{
				...
				{
					client_print(id, print_center, "You can not provide a working here works!")
				}
				else
				{
					ma_dzialko[id] = false;
