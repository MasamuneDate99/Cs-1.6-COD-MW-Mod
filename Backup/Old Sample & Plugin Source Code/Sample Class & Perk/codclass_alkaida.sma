...
new const opis[]    = "70 hp for killing enemy + bonus 300xp";
...
public DeathMsg()
{
    ...
    if(ma_klase[victim] && !ma_klase[killer])
        cod_set_user_xp(killer, cod_get_user_xp(killer)+300);
    
    if(ma_klase[killer])
    {
        new cur_health = pev(killer, pev_health);
        new Float:max_health = 100.0+cod_get_user_health(killer);
        new Float:new_health = cur_health+70.0<max_health? cur_health+70.0: max_health;
