...
new const nazwa[] = "Excellent";
new const opis[] = "1/3 chance to freeze the enemy";
new const bronie = (1<<CSW_GALIL) | (1<<CSW_FIVESEVEN);
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (g_NitrogenGalil[attacker] && get_user_weapon(attacker) == CSW_GALIL &&  random_num(1,3) == 1)
	{	
		if(cs_get_user_team(attacker) == cs_get_user_team(victim))
		return HAM_IGNORED
		
		set_pev(victim, pev_velocity, Float:{0.0,0.0,0.0}) // stop motion
		set_pev(victim, pev_maxspeed, 25.0) // prevent from moving
