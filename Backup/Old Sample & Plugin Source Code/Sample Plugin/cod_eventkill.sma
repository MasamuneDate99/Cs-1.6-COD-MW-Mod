...
public plugin_init() {
	...
	register_clcmd("say /eventkill", "Start");
	
	pCvarEvent = register_cvar("cod_eventczas", "1500");
	cvar_eventoff = register_cvar("cod_eventoff", "400");
	cvar_eventxp = register_cvar("cod_eventxp", "1200");
}

public Start(id)
{
	if(!(get_user_flags(id) & ADMIN_BAN) || task_exists( TASK_ID ) )
...
public Koniec()
{
	...
	show_hudmessage( 0, "^n^n^n[End Event for %d second!]|[3x more exp for Kill]", iTime )
