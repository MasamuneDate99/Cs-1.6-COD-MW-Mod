...
new min_lvl = 0;
new Float:procent = 0.06; // liczba 0.20 oznacza 20 % max lvl'u. Dajac 0.5 damy 50 % a 1.0 - 100 % lvlu
...
public plugin_init() 
{
	...
	cvar_limit_poziomu = register_cvar("cod_maxlevel", "1000"); 
...
public PokazInformacje(id) 
{
	...
	if(!is_user_alive(id))
	{
		...
		ShowSyncHudMsg(id, SyncHudObj, "INFO PLAYER:^n|Class : %s^n|Exp : %0.0f%%^n|Level : %i^n|Perk : %s^n|Perk2 : %s^n", nazwy_klas[klasa_gracza[target]], fProcent, poziom_gracza[target], nazwy_perkow[perk_gracza[0][target]], nazwy_perkow[perk_gracza[1][target]], inteligencja_gracza[target], kondycja_gracza[target]);//
		return PLUGIN_CONTINUE;
	}
...
public PokazReklame(id)
{
	...
	client_print(id, print_chat, "[COD:MW] Welcome to the COD:MW Sawah Hijau");
