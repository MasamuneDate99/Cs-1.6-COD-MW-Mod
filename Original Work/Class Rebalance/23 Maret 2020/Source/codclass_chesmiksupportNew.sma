...
#define nazwa "Chesmik Support"
#define opis "Charging knife, attack slow enemy by 50% (1/4) also deal fixed 10 poison damage * 3 tick each hit (1/1)"
...
public CurWeapon(id)
{
	...
	if(get_user_weapon(id) == CSW_KNIFE && !moc_zaladowana[id] && ma_klase[id])
	{
		...
		set_task(CZAS_LADOWANIA.0, "zatruwajacy", id+TASK_ID);
		set_bartime(id, CZAS_LADOWANIA);
	}
	else
	{
...
public Damage(id)
{
	...
	if(!moc_zaladowana[attacker])
		return;
	
	zatruwajacy[id] = attacker;
	if(!task_exists(id+TASK_ZATRUCIE) && random(100) <= 100)
		set_task(0.7, "Poison", id+TASK_ZATRUCIE, _, _, "a", 3);
...
public Poison(id)
{
	...
	client_print(zatruwajacy[id], print_center, "Poisoned Enemy !");
	cod_inflict_damage(zatruwajacy[id], id, 10.0, 0.0); //	10.0 Damage, 0.0 * Inteligence damage
}
