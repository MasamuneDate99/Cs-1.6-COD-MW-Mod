...
new const opis[] = "3 Lightning charge, 5s CD. Deal 0.5 * Total Inteligence ( E to use )";
...
public cod_class_skill_used(id) {
	...
	if(is_user_alive(ofiara)){
		...
		if(poprzednia_blyskawica[id]+5.0>get_gametime()) {
			client_print(id,print_chat,"Can be used after 5 Seconds");
			return PLUGIN_HANDLED;
		}
...
puscBlyskawice(id, ofiara, Float:fObrazenia = 55.0, Float:fCzas = 1.0){
	...
	cod_inflict_damage(id, ofiara, fObrazenia, 0.5, ent, DMG_SHOCK);		// Damage / Inteligence ( 0.5 )
