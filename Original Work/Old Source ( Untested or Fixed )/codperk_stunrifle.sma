...
new const name[] = "Stun Rifle";
new const desc[] = "Got Assault Rifle, which will stun enemy when reloading, dealing damage";
...
public plugin_init()
{
	register_plugin("[ZP] Stun Rifle", "1.0", "Raheem");
	...
	RegisterHam(Ham_Item_ItemSlot, "weapon_knife", "fw_ItemSlotKnife");
	...
	g_pCvarElecDmg = register_cvar("stunrifle_electricity_dmg", "30") // electricity damage - lowest damage for 1 bar only
...
public cod_perk_enable(id)
{
	...
	else
	{
		...
		ColorChat(id, GREEN, "Ported to COD by MasamuneDate", name);
		give_weapon_stunrifle(id);
		g_bHasStunRifle[id] = true;
	}
	return COD_CONTINUE;
}
...
public fw_ItemSlotKnife(const item)
{
...
public RechargeTask(id)
{
	...
	set_hudmessage(51, 153, 255, -1.0, -0.16, 0, 0.0, get_pcvar_float(g_pCvarBarFillTime) - 0.2)
	show_hudmessage(id, "Stun Rifle Battery: %s", szBars)
