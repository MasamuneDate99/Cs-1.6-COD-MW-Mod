...
public OpisKlasy(id)
{
	new menu = menu_create("Choose Class:", "OpisKlasy_Handle");
...
public OpisKlasy_Handle(id, menu, item)
{
	...
	format(opis, charsmax(opis), "\yClass: \w%s^n\yIntelligence: \w%i^n\yHealth: \w%i^n\yStrength: \w%i^n\yCondition: \w%i^n\yWeapons:\w%s^n\yAdditional description: \w%s^n%s", nazwy_klas[item], inteligencja_klas[item], zdrowie_klas[item], wytrzymalosc_klas[item], kondycja_klas[item], bronie, opisy_klas[item], opisy_klas[item][79]);
...
public WybierzKlase_Handle(id, menu, item)
{
	...
	if(klasa_gracza[id])
		client_print(id, print_chat, "[COD:MW] Class will be amended in the next round.");
	else
	{
		UstawNowaKlase(id);
		DajBronie(id);
...
public PrzydzielPunkty(id)
{
	...
	format(inteligencja, charsmax(inteligencja), "Intelligence: \r%i \y(Increases strength and skill class perks)", PobierzInteligencje(id, 1, 1, 1));
	format(zdrowie, charsmax(zdrowie), "Health: \r%i \y(Increases health)", PobierzZdrowie(id, 1, 1, 1));
	format(wytrzymalosc, charsmax(wytrzymalosc), "Strength: \r%i \y(Reduces injuries)", PobierzWytrzymalosc(id, 1, 1, 1));
	format(kondycja, charsmax(kondycja), "Condition: \r%i \y(Increases the pace of walking)", PobierzKondycje(id, 1, 1, 1));
	...
	format(kondycjaa, charsmax(kondycjaa), "Add 20 point cond");
	...
	{ 
		...
		{	
			if(kondycja_gracza[id] < limit_poziomu/2)
			{
				...
				client_print(id, print_chat, "[COD:MW] Maximum attained level of condition");
		}
		case 4: 
		{	
			if(inteligencja_gracza[id] < limit_poziomu/2)
		...
		{	
			...
			client_print(id, print_chat, "[COD:MW] ^x01 Maximum attained level of Condition");
		}
	}
...
public PokazInformacje(id) 
{
	...
	if(!is_user_alive(id))
	{
		...
		ShowSyncHudMsg(id, SyncHudObj, "INFO PLAYER:^n|Class : %s^n|Exp : %0.1f%%^n|Level : %i^n|Perk : %s^n|Perk2 : %s^n|Int : %i^n|Condition : %i", nazwy_klas[klasa_gracza[target]], fProcent, poziom_gracza[target], nazwy_perkow[perk_gracza[0][target]], nazwy_perkow[perk_gracza[1][target]], inteligencja_gracza[target], kondycja_gracza[target]);//
		return PLUGIN_CONTINUE;
	}
	...
	ShowSyncHudMsg(id, SyncHudObj, "[Class : %s]^n[Exp : %0.1f%%]^n[Lv : %i]^n[Perk : %s]^n[Perk2 : %s]^n[KS : x%d]^n[HP : %d]", nazwy_klas[klasa_gracza[id]], fProcent, poziom_gracza[id], nazwy_perkow[perk_gracza[0][id]], nazwy_perkow[perk_gracza[1][id]], licznik_zabiccod[id], hp);
