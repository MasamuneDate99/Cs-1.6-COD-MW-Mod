...
new min_lvl = 0;
new Float:procent = 0.04; // liczba 0.20 oznacza 20 % max lvl'u. Dajac 0.5 damy 50 % a 1.0 - 100 % lvlu
...
{	
	...
	if(get_user_team(id) != get_user_team(attacker) && klasa_gracza[attacker])
	{
		...
		else if(!perk_gracza[1][attacker] && get_user_flags(attacker) & ADMIN_LEVEL_E)
		UstawPerk(attacker, -1, -1, 1, 1);
		else if(!perk_gracza[1][attacker] && poziom_gracza[attacker] < 100)
		UstawPerk(attacker, -1, -1, 1, 1);
...
{
        new menu = menu_create("Select Class:", "WybierzKlase_Frakcje");
        for(new i=1; i <= ilosc_klas; i++)
        {
...
public WybierzKlase_Frakcje(id, menu, item)
{
        ...
        for(new i=1; i <= ilosc_klas; i++)
        {
                if(equali(data,frakcja_klas[i])){
                        WczytajDane(id, i);
                        format(klasa, charsmax(klasa), "%s \y[Level: %i]", nazwy_klas[i], poziom_gracza[id]);
                        num_to_str(i,szTmp,charsmax(szTmp));
                        menu_additem(menu2, klasa,szTmp);
                }
        }
...
public WybierzKlase_Handle(id, menu, item)
{
        ...
        if(klasa_gracza[id])
        client_print(id, print_chat, "[RAJAGAME] Class will be changed in the next round.");
        else
        {
                UstawNowaKlase(id);
                DajBronie(id);
...
public PrzydzielPunkty(id)
{
	...
	format(szybkosc, charsmax(szybkosc), "Much: \r[%d] \y(How many points to add )", co_ile[szybkosc_rozdania[id]]);
	format(inteligencja, charsmax(inteligencja), "Intelligence: \r%i \y(Increases Damage skills class perks)", PobierzInteligencje(id, 1, 1, 1));
	...
	format(kondycja, charsmax(kondycja), "Speed: \r%i \y(Increases walking speed)", PobierzKondycje(id, 1, 1, 1));
...
public PokazInformacje(id) 
{
	...
	if(!is_user_alive(id))
	{
		...
		{
			set_hudmessage(0, 255, 0, 0.02, 0.14, 0, 1.0, 1.0, 0.1, 0.1);
			ShowSyncHudMsg(id, SyncHudObj, "INFO VIP PLAYER:^n|Class : %s^n|Exp : %0.0f%%^n|Level : %i^n|Perk : %s^n|Perk2 : %s^n", nazwy_klas[klasa_gracza[target]], fProcent, poziom_gracza[target], nazwy_perkow[perk_gracza[0][target]], nazwy_perkow[perk_gracza[1][target]]);//
		}
		else{
			if(poziom_gracza[target] <= 100 )
			{
				set_hudmessage(0, 255, 0, 0.02, 0.14, 0, 1.0, 1.0, 0.1, 0.1);
				ShowSyncHudMsg(id, SyncHudObj, "INFO LOW PLAYER:^n|Class : %s^n|Exp : %0.0f%%^n|Level : %i^n|Perk : %s^n|Perk2 : %s^n", nazwy_klas[klasa_gracza[target]], fProcent, poziom_gracza[target], nazwy_perkow[perk_gracza[0][target]], nazwy_perkow[perk_gracza[1][target]]);//
			}
			if(poziom_gracza[target] > 100 )
			{
				set_hudmessage(0, 255, 0, 0.02, 0.14, 0, 1.0, 1.0, 0.1, 0.1);
				ShowSyncHudMsg(id, SyncHudObj, "INFO PLAYER:^n|Class : %s^n|Exp : %0.0f%%^n|Level : %i^n|Perk : %s^n", nazwy_klas[klasa_gracza[target]], fProcent, poziom_gracza[target], nazwy_perkow[perk_gracza[0][target]]);//
			}
		}
	...
	if(get_user_flags(id) & ADMIN_LEVEL_E)
	{
		...
		ShowSyncHudMsg(id, SyncHudObj, "[Class : %s]^n[Exp : %i | %0.0f%%]^n[LvL : %i]^n[Perk : %s]^n[Perk2 : %s]^n[KS : x%d]^n[HP : %d]", nazwy_klas[klasa_gracza[id]], doswiadczenie_gracza[id], fProcent, poziom_gracza[id], nazwy_perkow[perk_gracza[0][id]], nazwy_perkow[perk_gracza[1][id]], licznik_zabiccod[id], hp);
	}
	else{
		
		if(poziom_gracza[id] > 100){	
			set_hudmessage(0, 255, 0, 0.02, 0.23, 0, 0.0, 0.3, 0.0, 0.0);
			ShowSyncHudMsg(id, SyncHudObj, "[Class : %s]^n[Exp : %i | %0.0f%%]^n[LvL : %i]^n[Perk : %s]^n[KS : x%d]^n[HP : %d]", nazwy_klas[klasa_gracza[id]], doswiadczenie_gracza[id], fProcent, poziom_gracza[id], nazwy_perkow[perk_gracza[0][id]], licznik_zabiccod[id], hp);
		}
		if(poziom_gracza[id] < 100){
			set_hudmessage(0, 255, 0, 0.02, 0.23, 0, 0.0, 0.3, 0.0, 0.0);
			ShowSyncHudMsg(id, SyncHudObj, "[Class : %s]^n[Exp : %i | %0.0f%%]^n[LvL : %i]^n[Perk : %s]^n[Perk2 : %s]^n[KS : x%d]^n[HP : %d]", nazwy_klas[klasa_gracza[id]], doswiadczenie_gracza[id], fProcent, poziom_gracza[id], nazwy_perkow[perk_gracza[0][id]], nazwy_perkow[perk_gracza[1][id]], licznik_zabiccod[id], hp);
