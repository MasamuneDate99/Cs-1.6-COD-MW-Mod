...
#define MAX_ILOSC_PERKOW 300
#define MAX_ILOSC_KLAS 250
...
new min_lvl = 0;
new Float:procent = 0.05; // liczba 0.20 oznacza 20 % max lvl'u. Dajac 0.5 damy 50 % a 1.0 - 100 % lvlu
...
public plugin_init() 
{
	...
	cvar_doswiadczenie_za_obrazenia = register_cvar("cod_damagexp", "1"); // Exp each 20 HP
	...
	//register_me nucmd(register_me nuid("class:"), 1023, "OpisKlasy");
	...
	copy(opisy_perkow[0], MAX_WIELKOSC_OPISU, "To get an item, kill somebody or say /shop !");
...
public WybierzKlase_Handle(id, menu, item)
{
        ...
        if(klasa_gracza[id])
        client_print(id, print_chat, "[Sawah Hijau] Class will be changed in the next round.");
        else
        {
                UstawNowaKlase(id);
                DajBronie(id);
